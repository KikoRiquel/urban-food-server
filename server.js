...
app.post("/pedido", (req, res) => {
  ...
  db.beginTransaction((error) => {
    ...
    const query =
      'INSERT INTO Pedidos (fecha_pedido, total, estado, mesa_id, user_name) VALUES (CURRENT_TIMESTAMP, 0, "aceptado", ?, ?)';
    db.query(query, [mesa_id, user_name], (error, results) => {
      ...
      productos.forEach((producto) => {
        ...
        const query =
          "INSERT INTO DetallesPedidos (pedido_id, producto_id, cantidad, precio_unitario, ingredientes) VALUES (?, ?, ?, (SELECT precio FROM Productos WHERE producto_id = ?), ?)";
        db.query(
          query,
          [
            pedidoId,
...
app.get("/admin/pedidos/:id/ticket", authenticateToken, (req, res) => {
  ...
  db.query(queryPedido, [pedidoId], (error, results) => {
    ...
    doc
      .fontSize(14)
      .text(`Total: ${pedido.total.toFixed(2)}€`, { align: "right" });
